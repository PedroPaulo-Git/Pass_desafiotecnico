...
export function IncidentsSection({ vehicleId, incidents }: IncidentsSectionProps) {
  const { t } = useI18n()
        ...
        <CollapsibleTrigger asChild>
          ...
          >
            <div className="flex items-center gap-2">
              ...
              <span className="font-medium">{t.incidents.title}</span>
            </div>
            {isOpen ? (
        ...
        <CollapsibleContent>
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="p-4 pt-0">
                ...
                {incidents.length === 0 ? (
                  ...
                  incidents.map((incident) => (
                    <TableRow key={incident.id}>
                      ...
                      <TableCell>
                        <Badge className={getSeverityColor(incident.severity)}>{t.severity[incident.severity]}</Badge>
                      </TableCell>
                    </TableRow>
                  ))
                )}
            ...
            <div className="flex items-center justify-between mt-4 pt-4 border-t border-border">
              <span className="text-sm text-muted-foreground">{incidents.length}</span>
            </div>
