...
export function useVehicleDocuments(filters: DocumentFilters = {}) {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      if (filters.activeAlert !== undefined) params.append("activeAlert", String(filters.activeAlert))
      ...
      if (filters.expiringWithinDays !== undefined) params.append("expiringWithinDays", String(filters.expiringWithinDays))
      if (filters.sortBy) params.append("sortBy", filters.sortBy)
      if (filters.sortOrder) params.append("sortOrder", filters.sortOrder)
      if ((filters as any).search) params.append("search", (filters as any).search)

      const { data } = await api.get<PaginatedResponse<VehicleDocument>>(`/documents?${params.toString()}`)
      return data
    },
  })
}
...
export function useCreateVehicleDocument() {
  ...
  return useMutation({
    mutationFn: async (payload: any) => {
      const { data } = await api.post<VehicleDocument>(`/documents`, payload)
      return data
    },
    onSuccess: (data) => {
...
export function useUpdateVehicleDocument() {
  ...
  return useMutation({
    mutationFn: async ({ id, ...payload }: any) => {
      const { data } = await api.put<VehicleDocument>(`/documents/${id}`, payload)
      return data
    },
    onSuccess: (data) => {
      queryClient.invalidateQueries({ queryKey: ["documents"] })
