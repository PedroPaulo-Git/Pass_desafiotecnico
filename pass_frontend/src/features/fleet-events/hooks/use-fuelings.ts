...
export function useFuelings(filters: FuelingFilters = {}) {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      if (vehicleId) {
        const { data } = await api.get<Fueling[]>(
          `/vehicles/${vehicleId}/fuelings?sortBy=odometer&sortOrder=desc`
        );
        const items = data || [];
        return {
          items,
      ...
      if (filters.minOdometer !== undefined)
        params.append("minOdometer", String(filters.minOdometer));
      if (filters.maxOdometer !== undefined)
        params.append("maxOdometer", String(filters.maxOdometer));
      if (filters.minLiters !== undefined)
        params.append("minLiters", String(filters.minLiters));
      if (filters.maxLiters !== undefined)
        params.append("maxLiters", String(filters.maxLiters));
      if (filters.minUnitPrice !== undefined)
        params.append("minUnitPrice", String(filters.minUnitPrice));
      if (filters.maxUnitPrice !== undefined)
        params.append("maxUnitPrice", String(filters.maxUnitPrice));
      if (filters.totalValue !== undefined)
        params.append("totalValue", String(filters.totalValue));
      if (filters.sortBy) params.append("sortBy", filters.sortBy);
      if (filters.sortOrder) params.append("sortOrder", filters.sortOrder);
      if ((filters as any).search)
        params.append("search", (filters as any).search);

      const { data } = await api.get<PaginatedResponse<Fueling>>(
        `/fuelings?${params.toString()}`
      );
      return data;
    },
  });
