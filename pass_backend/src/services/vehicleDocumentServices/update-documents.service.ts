...
export const updateVehicleDocumentService = async (
  ...
  const existing = await prisma.vehicleDocument.findUnique({ where: { id: documentId.id } });
  if (!existing) {
    throw new AppError("Vehicle document not found", 404, "VEHICLE_DOCUMENT_NOT_FOUND", { documentId: documentId.id });
  }
