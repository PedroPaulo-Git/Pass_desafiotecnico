...
export async function vehicleImageRoutes(app: FastifyInstance) {
  ...
  app.get<{ Params: VehicleImageIdParam }>("/:id", controller.listImageById.bind(controller));
  app.put<{ Body: UpdateVehicleImageInput; Params: VehicleImageIdParam }>("/:id", controller.updateImage.bind(controller));
  app.delete<{ Params: VehicleImageIdParam }>("/:id", controller.deleteImage.bind(controller));
}
